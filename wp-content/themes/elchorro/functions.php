...
function show_custom_logo() {
	if ( $custom_logo_id = get_theme_mod( 'custom_logo' ) ) {
		$attachment_array = wp_get_attachment_image_src( $custom_logo_id, 'large' );
...
register_nav_menus( array(
	...
	'footer-mob-menu' => 'Footer Mobile Menu',
	'single-mob-menu' => 'Single Mobile Menu',
	) );
...
function foundation_scripts_and_styles() {
	if ( ! is_admin() ) {
		...
		wp_localize_script('global', 'myajax', array('url' => admin_url('admin-ajax.php')));
...
function hide_editor() {
  ...
  if($template_file == 'templates/template-menu.php'){ // the filename of the page template
  	remove_post_type_support('page', 'editor');
  }
}


/*Unwrap Contact Form 7 input*/
...
function my_action_callback() {
	...
	$gallery = get_field('gallery_page', $page_id);
	$gallery = array_slice($gallery, $count, 6);
	if ($gallery): ?>
...
add_action('wp_ajax_gallery', 'my_action_callback');
add_action('wp_ajax_nopriv_gallery', 'my_action_callback');
?>
<?php
